...
レシピを通すか止めるかの判定です。**開始時に 1 回、そのあと稼働中は毎 tick チェックされます。**

| タイミング | 満たしていないとき |
|---|---|
| レシピ開始時 | **開始しない。**材料は消費されません |
| 稼働中（毎 tick） | `conditionPolicy` に従う（既定は `pause`） |

`conditionPolicy` はレシピのトップレベルに書きます。`pause` なら条件が戻るまで止まって**続きから再開**し、
`abort` ならレシピを破棄します（**消費済みの材料は戻りません**）。書き方は
[JSON フォーマット](./JSON_FORMAT.md) の表を参照。
