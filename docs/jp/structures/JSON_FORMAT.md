...
| `speedMultiplier` | Float | 処理速度（デフォルト: 1.0） |
| `energyMultiplier` | Float | エネルギー消費（デフォルト: 1.0） |
...
```
レシピ側で `"requiredTier": { "glass": 2 }` と指定されている場合、上記の設定では `glass:2` 以上のブロックを使用している構造体でのみそのレシピが有効になります。
