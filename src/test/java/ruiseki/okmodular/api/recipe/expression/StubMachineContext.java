 ...
 * ============================================
 *
 * `IMachineState` は資源種ごとのメソッド（per-kind プリミティブ）を公開しており、
 * スタブはそれを全部実装するので大きい。
 *
 * **これは畳み残しではなく、意図した終点。** 汎用アクセサ 3 本は
 * interface の default メソッドとして per-kind プリミティブに振り分ける形にした。
 * 振り分けを実装クラス（MachineStateAgent）へ移すと per-kind プリミティブを
 * interface から外せるが、そうすると **このスタブが 3 本を直接実装することになり、
 * 振り分けを検証しているテスト（MachineState{Capacity,Amount,Space}Test）が
 * 自分自身を検証するだけになる**。同じ保証を得るにはコントローラと
 * ポートマネージャを組み立てる重いテストが必要になる。
 *
 * スタブが大きいことと引き換えに、振り分けが軽いテストで縛られている。
 *
 * ============================================
 */
